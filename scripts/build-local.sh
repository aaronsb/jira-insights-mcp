...
TEMP_DIR="/tmp/jira-insights-mcp"
mkdir -p "$TEMP_DIR"
...
if [ "$VERBOSE" = true ]; then
    if docker build -t jira-insights-mcp:local .; then
    ...
    if docker build -t jira-insights-mcp:local . > "$DOCKER_LOG" 2>&1; then
...
echo -e "\n${GREEN}Build complete!${NC} Image tagged as jira-insights-mcp:local"
